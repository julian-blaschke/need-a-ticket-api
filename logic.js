...
async function findOneUser({
  ...
  let selling = await findManyTickets({
    sellerId: _id
  })
  ...
  let bought = await findManyTickets({
    buyerId: _id
  })
...
async function findAndGroupTickets({
  concertId
}) {
  if (concertId) {
    let tickets = await Ticket.aggregate([{
      ...
      {
        $match: {
          "concert._id": Types.ObjectId(concertId)
        }
      },
    ])
    for (let ticket of tickets) {
      ticket.concert.artist = await findOneArtist({
        id: ticket.concert.artistId
      })
    }
    return tickets;
  }
...
//		inserts one artist
async function insertOneArtist({
  name
}) {
...
// 		insert one concert
async function insertOneConcert({
  title,
  date,
  address,
  genre,
...
//		insert one ticket
async function insertOneTicket({
  type,
  price,
  concertId,
  redeemedAt,
...
//		insert many tickets
async function insertManyTickets({
  amount,
  type,
  price,
  concertId,
...
async function buyOneTicket({
  ...
  let payerId = Types.ObjectId(userId)
...
async function buyManyTickets({
  ...
  let payerWallet = await findOneUser({
    id: payerId
  })
  ...
  let concert = Concert.findOne({
    _id: concertId.toString(),
...
async function redeemOneTicket({
  ticketId,
  user
}) {
